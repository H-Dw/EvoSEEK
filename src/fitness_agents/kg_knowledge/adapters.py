...
class ValidationKnowledgeAdapter:
    ...
    def extract(self, context: BuildContext) -> KnowledgeBatch:
        ...
        sample_reflections = tuple(context.resources.get("reflections", ()))
        ...
        if not all(isinstance(item, ReThinkReflection) for item in sample_reflections):
            raise TypeError(
                "resources['reflections'] must contain ReThinkReflection records"
            )
        ...
        entities: dict[str, EntityRecord] = {}
        relations: list[RelationRecord] = []
        sample_reflection_lookup = {
            item.reflection_id: item for item in sample_reflections
        }
        for sample_reflection in sample_reflections:
            entities[sample_reflection.reflection_id] = EntityRecord(
                sample_reflection.reflection_id,
                "ReThinkReflection",
                KnowledgeLayer.AGENT,
                frozenset({Modality.TEXT, Modality.TIME_SERIES}),
                {
                    "variant_id": sample_reflection.variant_id,
                    "round_id": sample_reflection.round_id,
                    "verdict": sample_reflection.verdict,
                    "summary": sample_reflection.summary,
                    "positive_findings": sample_reflection.positive_findings,
                    "negative_findings": sample_reflection.negative_findings,
                    "revised_reason": sample_reflection.revised_reason,
                    "next_round_advice": sample_reflection.next_round_advice,
                    "next_round_action": sample_reflection.next_round_action,
                    "provider": sample_reflection.provider,
                    "relation_scope": sample_reflection.relation_scope,
                    "assessment_id": sample_reflection.assessment_id,
                    "assessment_status": sample_reflection.assessment_status,
                    "assessment_commentary": sample_reflection.assessment_commentary,
                    "quality_status": sample_reflection.quality_status,
                    "advisory_only": sample_reflection.advisory_only,
                    "selection_eligible": sample_reflection.selection_eligible,
                    "dimension_assessments": sample_reflection.dimension_assessments,
                    "dimension_group_advice": sample_reflection.dimension_group_advice,
                },
                (f"agent:{sample_reflection.provider}",),
                "rethink_agent",
                valid_from_round=sample_reflection.round_id,
            )
        observation_lookup = {
        ...
        for record in records:
            ...
            if (
                ...
                and record.reflection_id in sample_reflection_lookup
            ):
                relations.append(
                    _relation(
                        self.name,
