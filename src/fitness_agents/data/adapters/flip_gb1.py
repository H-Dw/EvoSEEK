...
class FlipGB1Adapter:
    ...
    def canonicalize(self) -> CanonicalDataset:
        ...
        if self.spec.variant_column:
            required.add(self.spec.variant_column)
        ...
        for source_row_id, row in raw.iterrows():
            variant = str(
                row[self.spec.variant_column]
                if self.spec.variant_column
                else row[self.spec.sequence_column]
            ).strip().upper()
            components = (variant,)
            backbone_id = (
            ...
            model_sequence = str(
                row[self.spec.model_sequence_column]
                if self.spec.model_sequence_column
                else row[self.spec.sequence_column]
            ).strip()
            records.append(
                {
